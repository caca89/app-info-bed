...
class Datatable {
     ...
     * set the database (other than $active_group) - more info: http://Omeoo Media.com/forums/viewthread/145901/#712942
     */
            ...
            foreach ($filters as $filter) {
                if($filter['name'] != 'id') {
                    if($filter['name'] != 'actions') {
                        $sWhere .= $filter['name'] . " LIKE '%" . $search['value'] . "%' OR ";
                    }
                }
            }
    ...
    private function get_total_results($filtering = FALSE) {
        ...
        foreach ($this->joins as $val)
            $this->app->db->join($val[0], $val[1], $val[2]);

        foreach ($this->where as $val)
            $this->app->db->where($val[0], $val[1], $val[2]);

        foreach ($this->or_where as $val)
            $this->app->db->or_where($val[0], $val[1], $val[2]);

        foreach ($this->where_in as $val)
            $this->app->db->where_in($val[0], $val[1], $val[2]);

        foreach ($this->group_by as $val)
            $this->app->db->group_by($val);

        foreach ($this->like as $val)
            $this->app->db->like($val[0], $val[1], $val[2]);

        if (strlen($this->distinct) > 0) {
            $this->app->db->distinct($this->distinct);
            $this->app->db->select($this->columns);
        }

        $query = $this->app->db->get($this->table, NULL, NULL, FALSE);
        ...
        if (isset($custom_val['replacement']) && is_array($custom_val['replacement'])) {
            ...
            foreach ($custom_val['replacement'] as $key => $val) {
                ...
                if (preg_match('/(\w+::\w+|\w+)\((.*)\)/i', $val, $matches) && is_callable($matches[1])) {
                    ...
                    if ($sval == 'created_at' || $sval == 'updated_at' || $sval == 'deleted_at') {
                        $replace_string = $this->date_formated($row_data[($this->check_cType()) ? $sval : array_search($sval, $this->columns)]);
                    } else {
                        $replace_string = $row_data[($this->check_cType()) ? $sval : array_search($sval, $this->columns)];
                    }
                else
                    if ($sval == 'created_at' || $sval == 'updated_at' || $sval == 'deleted_at') {
                        $replace_string = $this->date_formated($sval);
                    } else {
                        $replace_string = $sval;
                    }
    ...
    public function date_formated($date) {
